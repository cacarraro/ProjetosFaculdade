...
public class Loja1 {
    private static Produto cama = new Produto("cama box casal", 100);
    private static Produto mesa = new Produto("mesa de jantar", 200);
    private static Produto armario = new Produto("armario duas portas de correr", 250);

    private static Produto[] catalogo = {cama, mesa, armario};

    // verifica se tem algum produto com a palavra procurada no catalogo dessa loja e adiciona na lista de resposta
        ...
        for (Produto p: catalogo)
        {
            ...
            for (String s: palabras)
            {
                if (s.equals(palavraParcial))
                {
                    ...
                    System.out.println("adicionou à resposta: " + p.getNome() + ", " + p.getPreco());
